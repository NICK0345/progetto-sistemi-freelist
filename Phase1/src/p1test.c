...
int main(void) {
    ...
    addokbuf("start removingBlocked   \n");
    p = removeBlocked(&sem[11]);
    addokbuf("finished removingBlocked   \n");


    if (insertBlocked(&sem[11], p))
        adderrbuf("removeBlocked: fails to return to free list   ");
    
    addokbuf("succeed to return to free list    \n");
    
    addokbuf("insertBlocked test #3 started  \n");
    if (insertBlocked(&onesem, procp[9]) == FALSE)
        adderrbuf("insertBlocked: inserted more than MAXPROaC   ");
